...
def index( request ):
    prodectList1 = Products.objects.filter(pruduct_keeper=request.session.get('user_name'))
    prodectList = prodectList1.filter(prodect_isdelete=0)
    pruduct_name = list(Products.objects.filter(Q(pruduct_keeper=request.session.get('user_name')) & Q(prodect_isdelete=0)).values_list("pruduct_name",flat=True))
    prodect_sales = list(Products.objects.filter(Q(pruduct_keeper=request.session.get('user_name')) & Q(prodect_isdelete=0)).values_list("prodect_sales",flat=True))
...
def product( request ):
    ...
    if request.method == "POST":
        ...
        product.pruduct_keeper = request.session.get('user_name')
        product.save()
